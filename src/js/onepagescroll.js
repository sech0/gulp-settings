	...
	_swipeEvents = function (el) {
		...
		function touchmove(event) {
			...
			function parallax() {
				...
				let slider = document.getElementById('ice');
				...
				let yPos = (window.pageYOffset - 800) / slider.dataset.speed;
				yPos = -yPos;
...
onePageScroll(".main", {
	...
	beforeMove: function (index) {
		if (index == 3) {
			document.getElementById("next__shadow").hidden = true;
			document.getElementById("next__text").classList.add('hide');
		}
		if (index == 2) {
			document.getElementById("next__shadow").hidden = false;
			document.getElementById("next__shadow").classList.remove('hide');
			document.getElementById("next__text").classList.remove('hide');
		}
	}
});